...
import { solveExchanger, correctionFactorF, correctionFactorFNShells, generateProfile, effectivenessTubosCorazaN, effectivenessFromNTU, overallUFromHiHo } from "./engine.js";
...
describe("Factor de corrección F para flujo cruzado (fix: antes se asumía F=1 siempre)", () => {
  it("recupera el área real a partir de las 4 temperaturas (construido hacia atrás, igual que el caso de coraza)", () => {
    // Se asume As_real=12.0, se derivan las 4 temperaturas que ESO produciría
    // (con la fórmula exacta de efectividad de cruzado-no-mezclado), y se
    // verifica que, dando esas 4 temperaturas (sin dar As), el motor recupera
    // As≈12.0. Antes (F=1 fijo) esto no se cumplía.
    const Ch = 300, Cc = 900, Cr = Ch / Cc, U = 55, As_real = 12.0;
    const NTU_real = (U * As_real) / Ch;
    const eps_real = effectivenessFromNTU("cruzado_no_mezclado", NTU_real, Cr);
    const Th_in = 200, Tc_in = 30;
    const Q = eps_real * Ch * (Th_in - Tc_in);
    const Th_out = Th_in - Q / Ch;
    const Tc_out = Tc_in + Q / Cc;

    const r = solveExchanger({
      tipo_intercambiador: "flujo_cruzado",
      configuracion_flujo: "cruzado_no_mezclado",
      fluido_caliente: { nombre: "caliente", temp_entrada_C: Th_in, temp_salida_C: Th_out, flujo_masico_kg_s: 1, cp_kJ_kgC: Ch / 1000 },
      fluido_frio: { nombre: "frio", temp_entrada_C: Tc_in, temp_salida_C: Tc_out, flujo_masico_kg_s: 1, cp_kJ_kgC: Cc / 1000 },
      coeficiente_U_W_m2C: U,
    });
    expect(r.F).toBeLessThan(0.999); // el F real de este caso NO es 1 (≈0.906)
    expect(r.As).toBeCloseTo(As_real, 4);
  });

  it("sin gastos másicos conocidos (no se puede saber Cr), cae de vuelta a F=1 con aviso explícito, sin reventar", () => {
    const r = solveExchanger({
      tipo_intercambiador: "flujo_cruzado",
      configuracion_flujo: "cruzado_no_mezclado",
      fluido_caliente: { nombre: "caliente", temp_entrada_C: 200, temp_salida_C: 120, cp_kJ_kgC: 1.05 },
      fluido_frio: { nombre: "frio", temp_entrada_C: 30, temp_salida_C: 90, cp_kJ_kgC: 4.18 },
      coeficiente_U_W_m2C: 55,
      area_m2: 12,
    });
    expect(r.error).toBeUndefined();
    expect(r.F).toBe(1);
    expect(r.warnings.some((w) => w.includes("flujo cruzado"))).toBe(true);
  });
});
